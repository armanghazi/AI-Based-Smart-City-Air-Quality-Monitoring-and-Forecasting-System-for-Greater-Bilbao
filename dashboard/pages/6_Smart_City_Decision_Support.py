...
{forecast_line}
    ...
    # v2 includes both buffer features (notebook 10a) and distance features (notebook 10b)
    SPATIAL_FILE = Path(__file__).parent.parent.parent / "data" / "processed" / "station_spatial_features_v2.csv"
    # Fallback to v1 if v2 not yet committed
    ...
    if not SPATIAL_FILE.exists():
        st.info(
            ...
            "Run `notebooks/10a_spatial_buffer_analysis.ipynb` and "
            "`notebooks/10b_distance_features.ipynb` locally to generate it, "
            "then commit `data/processed/station_spatial_features_v2.csv` to the repository."
        )
    else:
        # ── Load spatial data ──────────────────────────────────────────────────────
        @st.cache_data
        ...
        # ── Distance scatter plots (only shown when v2 file available) ────────────
        dist_col_centre = "dist_bilbao_centre_m"
        dist_col_port   = "dist_port_bilbao_m"

        if dist_col_centre in df_scatter.columns:
            st.markdown("#### Distance-Based Spatial Drivers")
            st.caption(
                "Straight-line (Haversine) distances to key landmarks. "
                "Negative trend = closer to source correlates with higher pollution."
            )
            sd1, sd2 = st.columns(2)

            with sd1:
                # Convert m → km for readability
                df_scatter["dist_centre_km"] = df_scatter[dist_col_centre] / 1000
                fig_sd1 = px.scatter(
                    df_scatter,
                    x="dist_centre_km", y="NO2",
                    text="station",
                    trendline="ols",
                    labels={"dist_centre_km": "Distance to City Centre (km)",
                            "NO2": "Mean NO₂ (µg/m³)"},
                    title="Distance to City Centre vs NO₂  (r = −0.77)",
                    color="zone",
                    color_discrete_map={
                        "Urban": "#8e44ad", "Industrial": "#e67e22",
                        "Port": "#2980b9", "Coastal": "#1abc9c", "Refinery": "#c0392b",
                ...
                fig_sd1.update_traces(textposition="top center",
                                      selector=dict(mode="markers+text"))
                fig_sd1.update_layout(height=350, margin=dict(t=50, b=10),
                                      showlegend=False)
                st.plotly_chart(fig_sd1, width="stretch")

            with sd2:
                df_scatter["dist_port_km"] = df_scatter[dist_col_port] / 1000
                fig_sd2 = px.scatter(
                    df_scatter,
                    x="dist_port_km", y="SO2",
                    text="station",
                    trendline="ols",
                    labels={"dist_port_km": "Distance to Port of Bilbao (km)",
                            "SO2": "Mean SO₂ (µg/m³)"},
                    title="Distance to Port vs SO₂  (r = −0.07, weak)",
                    color="zone",
                    color_discrete_map={
                        "Urban": "#8e44ad", "Industrial": "#e67e22",
                        "Port": "#2980b9", "Coastal": "#1abc9c", "Refinery": "#c0392b",
                ...
                fig_sd2.update_traces(textposition="top center",
                                      selector=dict(mode="markers+text"))
                fig_sd2.update_layout(height=350, margin=dict(t=50, b=10),
                                      showlegend=False)
                st.plotly_chart(fig_sd2, width="stretch")
                st.caption(
                    "⚠️ Port distance does not predict SO₂ — vessel emissions "
                    "are wind-driven, not distance-driven. Shown for completeness."
                )
        ...
        with st.expander("📋 Full spatial features table (all buffer radii)"):
            ...
            st.download_button(
                ...
                data=csv_sp,
                file_name="station_spatial_features_v2.csv",
                mime="text/csv",
            )
        ...
        st.info(
            ...
            "Landmark distances computed via Haversine great-circle formula. "
            "Correlations computed across n = 7 stations and are illustrative. "
            "Full analysis: `notebooks/10a_spatial_buffer_analysis.ipynb` · "
            "`notebooks/10b_distance_features.ipynb`."
        )
