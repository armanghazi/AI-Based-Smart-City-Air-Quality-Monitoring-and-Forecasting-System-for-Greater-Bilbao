...
Backend : Google AI Studio (gemini-2.0-flash)
API key : st.secrets["GEMINI_API_KEY"]  (Streamlit Cloud)
          or GEMINI_API_KEY env variable (local dev)
"""
...
def _tools_from_data(df, df_fc, gis, mdls):
    ...
    def _fd(name, description, properties, required):
        """Helper: build a Gemini FunctionDeclaration."""
        return gtypes.FunctionDeclaration(
            name=name,
            description=description,
            parameters=gtypes.Schema(
                type="OBJECT",
                properties={k: gtypes.Schema(**v) for k, v in properties.items()},
                required=required,
            ),
        )
    ...
    gemini_tool = gtypes.Tool(function_declarations=[
        _fd(
            "get_station_stats",
            "Get statistics (mean, max, min, std) for a pollutant or weather variable at a specific station",
            {
                "station":  {"type": "STRING"},
        ...
        _fd(
            "get_worst_days",
            "Get the days with the highest pollution or weather values",
            {
        ...
        _fd(
            "compare_all_stations",
            "Compare all stations by average value of a pollutant or weather variable",
            {"variable": {"type": "STRING"}},
            ["variable"],
        ),
        _fd(
            "get_yearly_trend",
            "Get yearly average trend of a variable for a station",
            {
                "station":  {"type": "STRING"},
                "variable": {"type": "STRING"},
            },
        ...
        _fd(
            "get_correlation",
            "Get Pearson correlation between a pollutant and a weather variable",
            {
                "pollutant":   {"type": "STRING"},
                "weather_var": {"type": "STRING"},
            },
            ["pollutant", "weather_var"],
        ),
        _fd(
            "forecast_pollutant",
            "Predict a pollutant level for a station on a specific date using XGBoost ML model",
            {
                "station":   {"type": "STRING"},
                "pollutant": {"type": "STRING", "description": "NO2, PM2.5, PM10, SO2"},
                "date":      {"type": "STRING", "description": "Date in YYYY-MM-DD format"},
            },
            ["station", "pollutant", "date"],
        ),
        _fd(
            "get_station_profile",
            "Get full GIS profile of a station: zone, elevation, land use percentages, distances to port/refinery/city",
            {"station": {"type": "STRING"}},
            ["station"],
        ),
        _fd(
            "get_spatial_comparison",
            "Compare all stations by a GIS feature. Use exact column names: elev_point_m, slope_deg, green_pct_1000m, industrial_pct_1000m, residential_pct_1000m, road_density_1000m, dist_port_bilbao_m, dist_petronor_m, dist_bilbao_centre_m",
            {"feature": {"type": "STRING", "description": "Exact GIS column name e.g. green_pct_1000m, industrial_pct_1000m, dist_petronor_m"}},
            ["feature"],
        ),
        _fd(
            "explain_pollution_by_geography",
            "Explain why a station has its pollution levels based on its geographic context (land use, distances, elevation)",
            {"station": {"type": "STRING"}},
            ["station"],
        ),
    ])

    schemas = gemini_tool  # single Tool object (not a list) — passed to GenerateContentConfig
...
def get_reply(history: list[dict], digest: str, tool_map: dict, schemas, stations: list) -> tuple[bool, str]:
    api_key = (
        st.secrets.get("GEMINI_API_KEY", None)
        or os.environ.get("GEMINI_API_KEY", None)
    )
    if not api_key:
        return False, (
            "The assistant is offline — no `GEMINI_API_KEY` configured. "
    ...
    try:
        ...
        cfg = gtypes.GenerateContentConfig(
            system_instruction=build_system_prompt(digest, stations),
            temperature=0.2,
        ...
        for _ in range(3):
            ...
            if not fn_calls:
                # No tool call — extract text and return
                text = "".join(
            ...
            for part in fn_calls:
                ...
                try:
                    ...
                    result = fn(**args) if fn else f"Tool '{fc.name}' not found."
                except Exception as e:
        ...
        # Safety net — ask for final answer without tools
        cfg_no_tools = gtypes.GenerateContentConfig(
            system_instruction=build_system_prompt(digest, stations),
            temperature=0.2,
            max_output_tokens=800,
        )
        resp = client.models.generate_content(model=MODEL, contents=contents, config=cfg_no_tools)
        text = "".join(
        ...
        return False, (
            f"The assistant could not answer right now. "
            f"Details: `{exc}`"
        )
...
st.caption(f"Data through **{freshness}** · {MODEL} via Google AI Studio")
