# pages/Home.py
# Platform homepage: Hero + Alert banner + Quick status + AI assistant CTA + Nav tiles
# Zone cards and city-wide trend charts moved to Daily Briefing to remove duplication.
...
def _get_tomorrow_exceedances() -> list:
    ...
    dashboard. An entry here means the model predicts Poor, Very poor, or
    Extremely poor for at least one pollutant at that station.
    """
    results = []
...
if n_exc == 0:
    ...
    st.markdown(
        ...
        f'{n_exc} {tr("EAQI")} <b>{worst_label}</b> {tr("forecasts for")} {forecast_str} · '
...
NAV = [
    {"icon": "🌅", "title": "Daily Briefing",
     "desc": "Zone breakdown, station risk ranking, next-day alerts, one-page PDF",
